 ...
 */
/*
 * $Id: mboxlist.c,v 1.94.4.6 1999/10/15 17:26:28 tmartin Exp $
 */
...
static void tmp_log_string(const char *str)
{
  FILE *stream;
			    ...
			    DB_TXN *tid)
{
	...
	*p = '\0';
	...
	case 0:
	  ...
	  parentacllen = strlen(mboxentry->acls); 
	...
	case EAGAIN:
	    ...
	    break;
	default:
	    tmp_log_string("error updating db\n");
...
struct auth_state *auth_state;
{
    ...
    printf("other stuff partition=%s\n",partition);
...
void* rock;
{
    ...
    *p = '\0';
	...
	while (r != DB_NOTFOUND) {
	    switch (r) {
	    case 0:
	      printf("found something in inbox.*\n");
		break;
