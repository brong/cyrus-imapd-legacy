...
binary-arch: build
	...
	install -o root -m 755 sieve/test debian/$basename/$basedir/bin/sieve-test
	...
	mkdir -p debian/$basename-build/usr/src/$basename
	find . -maxdepth 1 -mindepth 1 -not -name debian -print0 | \\
		xargs -0 -r -i cp -a {} debian/$basename-build/usr/src/$basename/
